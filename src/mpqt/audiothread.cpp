...
void AudioThread::setvolume(int test) {
m_player->setVolume(test);
}
...
AudioThread::AudioThread(QObject *parent) : QThread(parent) {
  ...
  connect(m_player, &QMediaPlayer::durationChanged, this,
          &AudioThread::durationChanged);
  connect(m_player, &QMediaPlayer::positionChanged, this,
          &AudioThread::positionChanged);
  // connect(m_player, &QMediaPlayer::positionChanged, this,
  // &AudioThread::seek);
  endOfMusic = true;
}
...
void AudioThread::play(QString filename,bool online) {
  ...
  0, 0))) qDebug() << "Can't play file:"+filename; else
  {
      ...
      emit startOfPlayback(BASS_ChannelBytes2Seconds(chan,
...
void AudioThread::signalUpdate() {
  if (endOfMusic == false) {
    playing = true;
    // emit curPos(BASS_ChannelBytes2Seconds(chan, BASS_ChannelGetPosition(chan,
    // BASS_POS_BYTE)),
    //   BASS_ChannelBytes2Seconds(chan, BASS_ChannelGetLength(chan,
    //   BASS_POS_BYTE)));
...
void AudioThread::changePosition(qint64 position) {
  // BASS_ChannelSetPosition(chan, BASS_ChannelSeconds2Bytes(chan, position),
  // BASS_POS_BYTE);
...
QString AudioThread::getDuration(QString path) {
  ...
  // return formattedTime(BASS_ChannelBytes2Seconds(chan,
  // BASS_ChannelGetLength(chan, BASS_POS_BYTE)));
  if (path.isNull()) {
    return "";
  }
